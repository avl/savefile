...
use crate::{
    abi_entry_light, parse_return_value_impl, AbiConnection, AbiConnectionMethod, AbiErrorMsg, AbiExportable,
    AbiProtocol, FlexBuffer, RawAbiCallResult, TraitObject,
};
use savefile::prelude::{
    get_schema, AbiMethod, AbiMethodArgument, AbiMethodInfo, AbiTraitDefinition, Deserialize, Deserializer,
    LittleEndian, ReadBytesExt, ReceiverType, SavefileError, Schema, Serialize, Serializer,
};
...
unsafe impl AbiExportable for dyn BufMut {
        ...
        AbiTraitDefinition {
            ...
            methods: vec![
                AbiMethod {
                    ...
                    info: AbiMethodInfo {
                        return_value: { get_schema::<usize>(version) },
                ...
                AbiMethod {
                    ...
                    info: AbiMethodInfo {
                        return_value: { get_schema::<()>(0) },
                        receiver: ReceiverType::Mut,
                        arguments: vec![AbiMethodArgument {
                            schema: { get_schema::<usize>(version) },
                        }],
                        async_trait_heuristic: false,
                    },
                },
    ...
    fn call(
        ...
        data: &[u8],
        abi_result: *mut (),
        __savefile_internal_receiver: unsafe extern "C" fn(outcome: *const RawAbiCallResult, result_receiver: *mut ()),
        ...
        match method_number {
            0u16 => {
                let ret = unsafe { &*trait_object.as_const_ptr::<dyn BufMut>() }.remaining_mut();
            ...
            1u16 => {
                ...
                unsafe { (&mut *trait_object.as_mut_ptr::<dyn BufMut>()).advance_mut(arg_cnt) };
            }
            2u16 => {
                let ret = unsafe { &mut *trait_object.as_mut_ptr::<dyn BufMut>() }.chunk_mut();
                let mut __savefile_internal_data = FlexBuffer::new();
                let mut serializer = Serializer {
                    writer: &mut __savefile_internal_data,
                    file_version: 0u32,
...
unsafe impl BufMut for AbiConnection<dyn BufMut> {
    ...
    fn remaining_mut(&self) -> usize {
        ...
        let Some(callee_method_number) = info.callee_method_number else {
            panic!("Method \'{0}\' does not exist in implementation.", info.method_name,);
        };
        let mut result_buffer = MaybeUninit::<Result<usize, SavefileError>>::uninit();
        ...
        unsafe {
            unsafe extern "C" fn abi_result_receiver(outcome: *const RawAbiCallResult, result_receiver: *mut ()) {
                let outcome = unsafe { &*outcome };
                let result_receiver =
                    unsafe { &mut *(result_receiver as *mut std::mem::MaybeUninit<Result<usize, SavefileError>>) };
                result_receiver.write(parse_return_value_impl(
                    outcome,
    ...
    unsafe fn advance_mut(&mut self, arg_cnt: usize) {
        ...
        let mut result_buffer = MaybeUninit::<Result<(), SavefileError>>::new(Ok(()));
        let compatibility_mask = info.compatibility_mask;
        ...
        unsafe {
            unsafe extern "C" fn abi_result_receiver(outcome: *const RawAbiCallResult, result_receiver: *mut ()) {
                let outcome = unsafe { &*outcome };
                let result_receiver =
                    unsafe { &mut *(result_receiver as *mut std::mem::MaybeUninit<Result<(), SavefileError>>) };
                result_receiver.write(parse_return_value_impl(outcome, |_| -> Result<(), SavefileError> {
                    Ok(())
                }));
            }
            (self.template.entry)(AbiProtocol::RegularCall {
    ...
    fn chunk_mut(&mut self) -> &mut UninitSlice {
        ...
        let mut result_buffer = MaybeUninit::<Result<&mut UninitSlice, SavefileError>>::uninit();
        let compatibility_mask = info.compatibility_mask;
        let mut __savefile_internal_datarawdata = [0u8; 4usize];
        let mut __savefile_internal_data = Cursor::new(&mut __savefile_internal_datarawdata[..]);
        let mut serializer = Serializer {
        ...
        unsafe {
            unsafe extern "C" fn abi_result_receiver(outcome: *const RawAbiCallResult, result_receiver: *mut ()) {
                ...
                let result_receiver = unsafe {
                    &mut *(result_receiver as *mut std::mem::MaybeUninit<Result<&mut UninitSlice, SavefileError>>)
                };
                result_receiver.write(parse_return_value_impl(
                    outcome,
