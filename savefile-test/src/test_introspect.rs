...
pub enum EnumWithName {
    Variant1(#[savefile_introspect_key] String),
    Variant2 {
        #[savefile_introspect_key]
        name: String,
        value: String,
...
pub fn test_simple_enum_with_key() {
    ...
    let var2 = EnumWithName::Variant2 {
        name: "James".into(),
        value: "IV".into(),
    };
...
#[test]
pub fn do_test1() {
    let test = SimpleStruct { item1: 342 };
...
pub fn do_test_rwlock() {
    let test = RwLock::new(SimpleStruct { item1: 342 });
    ...
    assert_eq!(
        test.introspect_child(0).unwrap().val().introspect_value(),
        "SimpleStruct"
    );
...
pub fn do_test_mutex() {
    let test = Mutex::new(SimpleStruct { item1: 343 });
...
pub fn test_introspect_no_children() {
    ...
    assert_eq!(
        base_introspector
            ...
            .unwrap_err(),
        IntrospectionError::NoChildren
    );
}
#[test]
pub fn test_introspector_simpler_case0() {
    let comp = ComplexStruct {
        simple1: SimpleStruct { item1: 37 },
        simple2: SimpleStruct { item1: 38 },
        an_int: 4,
    };

    let mut base_introspector = Introspector::new();

    let result = base_introspector
        .do_introspect(
            &comp,
            IntrospectorNavCommand::SelectNth {
                select_depth: 0,
    ...
    let result = base_introspector
        ...
        .expect("Leafs should also be selectable");
...
pub fn test_introspector_simpler_case1() {
    ...
    let result = base_introspector
        .do_introspect(
            &comp,
            IntrospectorNavCommand::ExpandElement(IntrospectedElementKey {
                key_disambiguator: 0,
...
pub fn test_introspector_simple_case_ignored_field() {
    ...
    {
        let result = base_introspector
            .do_introspect(&comp, IntrospectorNavCommand::Nothing)
            .unwrap();
...
pub fn test_introspector_simple_case() {
...

    {
        ...
        assert_eq!(
            base_introspector
                .do_introspect(&comp, IntrospectorNavCommand::Up)
                .unwrap_err(),
            IntrospectionError::AlreadyAtTop
        );
        assert_eq!(
            base_introspector
                .do_introspect(
                ...
                .unwrap_err(),
            IntrospectionError::IndexOutOfRange
        );
        assert_eq!(
            base_introspector
                .do_introspect(
...
pub fn test_introspector_deeply_nested_case() {
    let sample = NestableStruct {
        a: Some(Box::new(NestableStruct {
            a: Some(Box::new(NestableStruct { a: None, b: None, c: 1 })),
            b: Some(Box::new(NestableStruct { a: None, b: None, c: 2 })),
            c: 3,
        })),
        b: Some(Box::new(NestableStruct {
            a: Some(Box::new(NestableStruct { a: None, b: None, c: 4 })),
            b: Some(Box::new(NestableStruct { a: None, b: None, c: 5 })),
            c: 6,
        })),
        c: 7,
    };
