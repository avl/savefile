...
impl<'a> Read for CryptoReader<'a> {
    fn read(&mut self, buf: &mut [u8]) -> Result<usize, Error> {
        loop {
                    ...
                    Ok(gotsize) => {
                        if gotsize == 0 {
                            if sizebuf_bytes_read == 0 {
                                ...
                                buf[0..cur_content_size].clone_from_slice(
                                    &self.buf[self.offset..self.offset + cur_content_size],
                                );
            ...
            self.reader
                .read_exact(&mut self.buf[orglen..orglen + curlen])?;
...
impl<'a> Write for CryptoWriter<'a> {
    ...
    fn flush(&mut self) -> Result<(), Error> {
        ...
        while self.buf.len() > offset {
            ...
            match self
                .sealkey
                .seal_in_place_append_tag(aead::Aad::empty(), curbuf)
            {
                Ok(_) => {}
                Err(_) => {
                    return Err(Error::new(ErrorKind::Other, "Cryptography error"));
...
fn diff_schema(a: &Schema, b: &Schema, path: String) -> Option<String> {
    let (atype, btype) = match *a {
        ...
        Schema::Undefined => {
            return Some(format!(
                "At location [{}]: Undefined schema encountered.",
                path
            ));
        }
...
pub mod array_impls {
    use crate::{
        introspect_item, Deserialize, Deserializer, Introspect, IntrospectItem, SavefileError,
        Schema, SchemaArray, Serialize, Serializer, WithSchema,
    };
    use std::mem::MaybeUninit;
    ...
    fn deserialize(deserializer: &mut Deserializer) -> Result<Self, SavefileError> {
        if !T::repr_c_optimization_safe(deserializer.file_version) {
...

            {
                ...
                let slice: &mut [MaybeUninit<u8>] = unsafe {
                    std::slice::from_raw_parts_mut(ptr as *mut MaybeUninit<u8>, num_bytes)
                };
                deserializer
                    .reader
                    .read_exact(unsafe { std::mem::transmute(slice) })?;
            }
            let ptr = &mut data as *mut _ as *mut [T; N];
            let res = unsafe { ptr.read() };
            core::mem::forget(data);
...
impl<V: Deserialize, T: arrayvec::Array<Item = V>> Deserialize for arrayvec::ArrayVec<T> {
    default fn deserialize(
        deserializer: &mut Deserializer,
    ) -> Result<arrayvec::ArrayVec<T>, SavefileError> {
    ...
    fn deserialize(
        ...
        if l > ret.capacity() {
            return Err(SavefileError::ArrayvecCapacityError {
                msg: format!(
                    "ArrayVec with capacity {} can't hold {} items",
                    ret.capacity(),
                    l
                ),
            });
        ...
        if !V::repr_c_optimization_safe(deserializer.memory_version) {
            ...
            unsafe {
                let bytebuf = std::slice::from_raw_parts_mut(
...
impl<T1: Introspect, T2: Introspect, T3: Introspect, T4: Introspect> Introspect
    for (T1, T2, T3, T4)
{
    fn introspect_value(&self) -> String {
    ...
    fn dive<'a>(
        ...
        loop {
            if let Some(child_item) = object.introspect_child(index) {
                ...
                let disambig_counter: &mut usize =
                    key_disambig_map.entry(key.clone()).or_insert(0usize);
                ...
                if let Some(cur_path_obj) = &cur_path {
                        ...
                        && cur_path_obj.key_disambiguator == *disambig_counter
                    {
                        ...
                        if has_children {
                            let mut subresult = self.dive(
                                depth + 1,
                                child_item.val(),
                                navigation_command.take().unwrap(),
                            )?;
